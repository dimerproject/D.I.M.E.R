...
// estos son los engranajes que quiero
...
module meshing_double_helix ()
{
    ...
    mio ();
	mirror ([0,1,0])
	translate ([0,0,0])//([28,0,3,5])
	mio3 (number_of_teeth=19);
}
	...
	teeth=38)
{
	//double helical gear
	{
		twist=200; //inclinacion dientes
		height=15; // altura dientes
		pressure_angle=30;

		gear (number_of_teeth=76,
			circular_pitch=105, //diametro 
			pressure_angle=20,
			clearance = 0.8,
			gear_thickness = height/4*0.5,//altura centro engranaje
			rim_thickness = height/2,
			rim_width = 1, //grosor borde anterior dientes
			hub_thickness = height/2*1.2,
			hub_diameter=5,//diametro buje centro
			bore_diameter=5,
			circles=circles,
			twist=0);
		mirror([0,0,1])
        //modificacion mia, es el disco de abajo
...
difference(){
    ...
    translate([0,0,-20])
    cylinder(r=2.5, h=100, $fn=25);
    }




//placa soporte ejes engranajes
*difference(){
    cube([50,10,2], center=true);
    union(){
        translate([20,0,0])
        cylinder(r=2, h=20, $fn=25, center=true);
        translate([-20,0,0])
        cylinder(r=2, h=20, $fn=25, center=true);
    }
}
